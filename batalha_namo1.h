...
int escolha_aleatoria(int LimI, int LimS)
{
    int num;
    ...
    int mostra_batalha()
{
    ...
    printf("\n\tHP:\t\t%d", jogador.hp);
    printf("\n\n\tOPONENTE:\t%s", namo1.nome);
    printf("\n\tGênero:\t\t%s", namo1.genero);
    printf("\n\tHP:\t\t%d", namo1.hp);
}
...
int mostra_defesas()
{
    ...
    if (escolha == 1 && escolha_pc == 2) {namo1.hp = namo1.hp - 30;} else
    if (escolha_pc == 1 && escolha == 2) {jogador.hp = jogador.hp -30;} else
    if (escolha == 1 & escolha_pc == 1) {namo1.hp = namo1.hp - 30; jogador.hp = jogador.hp - 30;} else
    if (escolha == 2 & escolha_pc == 2) {namo1.hp = namo1.hp - 0; jogador.hp = jogador.hp - 0;};

    if(ataque == 1 && defesa_pc == 1 && ataque_pc == 0 && defesa == 0) {namo1.hp = namo1.hp+30;} else
    if(ataque == 2 && defesa_pc == 2 && ataque_pc == 0 && defesa == 0) {namo1.hp = namo1.hp+30;} else
    if(ataque == 3 && defesa_pc == 3 && ataque_pc == 0 && defesa == 0) {namo1.hp = namo1.hp+30;} else
    ...
    }while (namo1.hp>0 && jogador.hp>0);

    if(jogador.hp>0 && namo1.hp<0){
    system("cls");
    printf("\n\n\n\tFIM DA BATALHA");
    printf("\n\tO nível de HP atual de %s é %d", jogador.nome, jogador.hp);
    printf("\n\tO nível de HP atual de %s é %d", namo1.nome, namo1.hp);
    Sleep(3000);
    morte = 1;
    } else

    if(namo1.hp>0 && jogador.hp<0){
    system ("cls");
